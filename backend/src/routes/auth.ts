...
router.post('/register', async (req, res) => {
  try {
    ...
    const initialPlan = finalRole === 'agency' ? 'agency' : 'free';

    const result = await db.query(
      `INSERT INTO users (email, password_hash, name, username, phone, phone_country, role, plan, is_verified)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, email, name, username, role, plan, avatar_url, is_verified, bio`,
      [email.toLowerCase(), passwordHash, name, username, phone, phoneCountry, finalRole, initialPlan, isVerified]
    );
