...
function lsGet<T>(key: string, fallback: T): T {
  try {
    ...
    // Support both JSON values written by this store and older raw values
    // such as localStorage.setItem('skilltok_is_authenticated', 'true').
    try { return JSON.parse(v) as T; } catch { return v as unknown as T; }
  } catch { return fallback; }
}
function lsAuth(): boolean {
  const v = lsGet<boolean | string>(LS_AUTH, false);
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  currentTab: lsAuth() ? 'feed' : 'auth' as any,
  ...
  login: (email, password, requestedRole) => {
    if (get().isLiveApiMode) {
      BackendService.login(email, password).catch(() => {});
    }
    const newUser: User = {
  ...
  register: (name, email, password, role, adminSecret) => {
    ...
    if (get().isLiveApiMode) {
      BackendService.register(name, email, password, finalRole, undefined, adminSecret).catch(() => {});
    }
    const newUser: User = {
      id: 'u-' + Date.now(),
      email,
  ...
  videos: lsGet<Video[]>(LS_VIDEOS, []).filter(v => !!v.videoUrl && !String(v.videoUrl).startsWith('blob:')),
  services: [],
  creators: [],
  ...
  deliverOrder: (txId, proofUrl, notes) => {
    ...
    set((state) => {
      const updatedTx = state.transactions.map(t =>
        t.id === txId ? { ...t, status: 'delivered' as const, deliveryProofUrl: proofUrl, deliveryNotes: notes, deliveredAt, autoReleaseDate: autoRel } : t
      );
      lsSet(LS_TRANSACTIONS, updatedTx);
      return {
        transactions: updatedTx,
  ...
  disputeOrder: (txId, reason) => {
    set((state) => {
      const updatedTx = state.transactions.map(t =>
        t.id === txId ? { ...t, status: 'disputed' as const, disputeReason: reason } : t
      );
      lsSet(LS_TRANSACTIONS, updatedTx);
      return {
        transactions: updatedTx,
