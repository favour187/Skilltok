...
export const BackendService = {
  ...
  addComment: async (videoId: string, content: string) => {
    const res = await api.post(`/api/videos/${videoId}/comments`, { content });
    return res.data;
  },

  // ============ PAYMENTS (Paystack webhook verification required in production) ============
  ...
  getThread: async (otherUserId: string) => {
    const res = await api.get(`/api/messages/thread/${otherUserId}`);
    return res.data;
  },
